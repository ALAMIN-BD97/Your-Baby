...
const API_URL = "https://nix-random-api.vercel.app/api/video/mixvideo";
...
module.exports.onStart = async function ({ api, event }) {
  ...
  try {
    ...
    const videoUrl = data.url.url;
    const name = data.url.name;
    const cp = data.cp;
    ...
    const filePath = path.join(cacheDir, `rndm_${Date.now()}.mp4`);
    ...
    const responseMsg = `${cp}\n\n𝐓𝐨𝐭𝐚𝐥 𝐕𝐢𝐝𝐞𝐨𝐬: [${total}]\n𝐀𝐝𝐝𝐞𝐝 𝐓𝐡𝐢𝐬 𝐕𝐢𝐝𝐞𝐨 𝐓𝐨 𝐓𝐡𝐞 𝐀𝐩𝐢 𝐁𝐲 [${name}]`;

    return api.sendMessage(
      { body: responseMsg, attachment: fs.createReadStream(filePath) },
      threadID,
